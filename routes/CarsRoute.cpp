...
std::function<void(const httplib::Request&, httplib::Response&)> CarsRoute::BandsGet()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
...

        {
            rapidjson::Value v;
            v.SetObject();
            v.AddMember("name", "subzero", d.GetAllocator());
            v.AddMember("image", "data/bands/subzero.webp", d.GetAllocator());
            d.PushBack(v, d.GetAllocator());
        }

        {
            rapidjson::Value v;
            v.SetObject();
            v.AddMember("name", "scorpion", d.GetAllocator());
            v.AddMember("image", "data/bands/scorpion.webp", d.GetAllocator());
            d.PushBack(v, d.GetAllocator());
        }

        {
            rapidjson::Value v;
            v.SetObject();
            v.AddMember("name", "ramtornado", d.GetAllocator());
            v.AddMember("image", "data/bands/ramtornado.webp", d.GetAllocator());
            d.PushBack(v, d.GetAllocator());
        }
...

        {
            rapidjson::Value v;
            v.SetObject();
            v.AddMember("name", "hulk", d.GetAllocator());
            v.AddMember("image", "data/bands/hulk.webp", d.GetAllocator());
            d.PushBack(v, d.GetAllocator());
        }
...
void CarsRoute::_ParseFilter(const httplib::Request& req, CarFilter& filter)
{
    ...
    int mileageFrom = -1;
    if (req.has_param("mileage_from")) mileageFrom = atoi(req.get_param_value("mileage_from", 0).c_str());
    int mileageTo = -1;
    if (req.has_param("mileage_to")) mileageTo = atoi(req.get_param_value("mileage_to", 0).c_str());
